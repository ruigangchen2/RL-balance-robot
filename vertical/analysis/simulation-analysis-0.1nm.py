...
policy.load_state_dict(
    torch.load('C:/Users/coty/Documents/RL-balance-robot/vertical/training-1nm/outputs/PPO_vertical_0_1NM-2.pth'))
policy.to('cpu')
...
class PendulumEnv:
    ...
    def step(self, act_index):
        ...
        self.steps += 1
        if abs(self.state[0] - thtb_target) < settle_tb and abs(self.state[1] - dthtb_target) < settle_dtb * 50 and abs(self.state[2] - dthtb_target) < settle_dtw * 100:
            self.reward = reward_scale  # 如果达到了目标，那么奖励5
            success.append(1)
            self.over = True
        elif abs(self.state[0]) > theta_nondim * 1.0: #and abs(self.steps) > 50:
...
plt.yticks([])
plt.axis('off')
vertical_line_5 = [1] * (count_time + 1)
vertical_line_5_ = [-1] * (count_time + 1)  # * np.pi / 180
start_point = 0
plt.subplot(3, 1, 1)
