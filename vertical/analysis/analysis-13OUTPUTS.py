...
policy = torch.nn.Sequential(torch.nn.Linear(3, nnn * 2), torch.nn.Tanh(),
                             ...
                             torch.nn.Linear(nnn, 13), torch.nn.Softmax(dim=1))
policy.load_state_dict(
    torch.load('C:/Users/Administrator/Desktop/Cases/RL-balance-robot/vertical/training/13outputs/PPO_vertical.pth'))
policy.to('cpu')
...
class PendulumEnv:
    ...
    def step(self, act_index):
        ...
        self.steps += 1
        if abs(self.state[0] - thtb_target) < settle and abs(self.state[1] - dthtb_target) < settle * 80 and abs(
                self.state[2] - dthtw_target) < settle * 300:
        ...
        self.reward -= (abs(self.steps) * 0.07 + abs(action) * 0.1)
...
plt.plot(np.array(range(count_time-1)) * dt, thetas1[1:], 'b-+', label=r'$\theta_b$')
...
plt.xlim([0, count_time * dt])
plt.plot(np.array(range(count_time-1)) * dt, dthetas1[1:], 'b-', label=r'$\dot{\theta}_b$')
plt.plot(np.array(range(count_time-1)) * dt, dthetas2[1:], 'r-', label=r'$\dot{\theta}_w$')
plt.gca().add_patch(Rectangle((0, vertical_line_5_[start_point]), dt * (count_time + 1),
                              vertical_line_5[start_point] - vertical_line_5_[start_point],
                              edgecolor='none', facecolor=[1, 0, 0], alpha=0.2))
plt.legend(ncol=2)
