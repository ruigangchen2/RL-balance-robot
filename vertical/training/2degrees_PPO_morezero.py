...
model.load_state_dict(
    torch.load('./outputs/PPO_vertical_critic_morezero_2.pth'))
policy.load_state_dict(
    torch.load('./outputs/PPO_vertical_morezero_2.pth'))
...
class PendulumEnv:
    ...
    def step(self, act_index):
        ...
        self.steps += 1
        if abs(self.state[0] - thtb_target) < settle and abs(self.state[1] - dthtb_target) < settle * 10 and abs(self.state[2] - dthtw_target) < settle * 150:
            self.reward = reward_scale  # 如果达到了目标，那么奖励5
            success.append(1)
            self.over = True
        elif abs(self.state[0]) > theta_nondim * 1.2 and abs(self.steps) > 40:
            self.reward = -reward_scale * 5  # 施加惩罚
            self.over = True
        else:
            self.reward = 0
            self.over = False
        self.reward -= (abs(self.steps) * 0.1 + abs(action) * 0.5)
...
for epoch in range(episode):
    ...
    if epoch > 1:
        ...
        if ini_a >= ini_b:
            if os.path.exists(
                    f'./outputs/PPO_vertical_morezero_{ini_b}.pth'):
                os.remove(f'./outputs/PPO_vertical_morezero_{ini_b}.pth')
            if os.path.exists(
                    f'./outputs/PPO_vertical_morezero_critic_{ini_b}.pth'):
                os.remove(f'./outputs/PPO_vertical_morezero_critic_{ini_b}.pth')
            torch.save(policy.state_dict(),
                       f'./outputs/PPO_vertical_morezero_{ini_a}.pth')
            torch.save(model.state_dict(),
                       f'./outputs/PPO_vertical_morezero_critic_{ini_a}.pth')
            ini_b = ini_a
    ...
    torch.save(policy.state_dict(), f'./outputs/PPO_vertical_morezero.pth')
    torch.save(model.state_dict(), f'./outputs/PPO_vertical_critic_morezero.pth')
