...
simu_time = 1.5  # 总的仿真时间长度，除以dt是进行了多少步
success = []
...
policy1.load_state_dict(torch.load('C:/Users/coty/Documents/RL-balance-robot/vertical/training-small-range/outputs/PPO_vertical_small_range_more_zero-3.pth'))
policy1.to(device)
    ...
    for j in range(N2):
        for k in range(N3):
            ...
            for step in range(steps):
                ...
                if abs(y[0] - thtb_target) < settle_tb and abs(y[1] - dthtb_target) < settle_dtb * 8 and abs(y[2] - dthtb_target) < settle_dtw * 15:
                    flag += 1
...
for ii in range(len(thtab)):
    ...
    plt.title(r"{:.1f}$^\circ$".format(thtab[ii] * 180 / np.pi), fontsize=5)
