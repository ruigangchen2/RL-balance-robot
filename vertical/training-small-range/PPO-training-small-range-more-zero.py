...
model.load_state_dict(
    torch.load('outputs/PPO_vertical_critic_small_range_more_zero-3.pth'))
policy.load_state_dict(
    torch.load('outputs/PPO_vertical_small_range_more_zero-3.pth'))
...
class PendulumEnv:
    ...
    def step(self, act_index):
        ...
        self.steps += 1
        if abs(self.state[0] - thtb_target) < settle_tb and abs(self.state[1] - dthtb_target) < settle_dtb * 8 and abs(self.state[2] - dthtb_target) < settle_dtw * 15:
            ...
            self.reward = 0
            self.over = False
        self.reward -= (abs(self.steps) * 0.1 + abs(action) * 50)
...
for epoch in range(episode):
    ...
    if epoch > 1:
        ...
        if ini_a >= ini_b:
            if os.path.exists(
                    f'./outputs/PPO_vertical_small_range_more_zero{ini_b}.pth'):
                os.remove(f'./outputs/PPO_vertical_small_range_more_zero{ini_b}.pth')
            if os.path.exists(
                    f'./outputs/PPO_vertical_critic_small_rang_more_zeroe{ini_b}.pth'):
                os.remove(f'./outputs/PPO_vertical_critic_small_range_more_zero{ini_b}.pth')
            torch.save(policy.state_dict(),
                       f'./outputs/PPO_vertical_small_range_more_zero{ini_a}.pth')
            torch.save(model.state_dict(),
                       f'./outputs/PPO_vertical_critic_small_range_more_zero{ini_a}.pth')
            ini_b = ini_a
    ...
    torch.save(policy.state_dict(), f'outputs/PPO_vertical_small_range_more_zero-3.pth')
    torch.save(model.state_dict(), f'outputs/PPO_vertical_critic_small_range_more_zero-3.pth')
